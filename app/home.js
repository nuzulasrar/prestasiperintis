import { Text, View, ScrollView, TextInput, TouchableOpacity, FlatList, useWindowDimensions } from 'react-native'
...
export default function Page() {
    ...
    const getProjectList = async () => {
        ...
        fetch('http://prestasiperintis.nuzul.com.my/api/project')
            .then(res => res.json())
            .then(json => {
                setProjects(json)
        ...
        return (
            <TouchableOpacity
                ...
                <View className="">
                    ...
                    <Text className="text-black text-[16px] mb-2 font-bold"> Type: {item.project_type}</Text>
                    <Text className="text-black text-[16px]"> Year: {item.project_name}</Text>
                    <Text className="text-black text-[16px]"> Highway: {item.project_name}</Text>
                    <Text className="text-black text-[16px]"> Highway Code: {item.project_name}</Text>
                </View>
                <View className="">
        ...
        <View className="flex-1 justify-center bg-gray-200">
            ...
            <View className="w-11/12 self-center mb-5">
                ...
                <View className="flex-row justify-between items-center w-full">
                    <TextInput className="bg-white w-9/12 mt-6 mb-6 h-[50px] text-black pl-5 rounded-l-full" placeholder='Search Project' placeholderTextColor={"black"} />
            ...
            <FlatList
                data={projects}
                renderItem={({ item, index }) => <ProjectItems item={item} index={index} />}
            />
