import { Text, View, ScrollView, TextInput, TouchableOpacity, FlatList, Button, Image } from 'react-native'
...
export default function Page() {
    ...
    const getBridgeList = async () => {
        try {
            fetch("http://192.168.56.1:3000/api/bridgelist")
                .then(res => res.json())
                .then(json => {
                    setBridges(json)
    ...
    const FormItems = ({ item, index }) => {
                        ...
                        return (
                            <View className="ml-2 mb-2">
                                <View className="flex-row items-center p-2 bg-emerald-400 rounded-lg mb-2">
                                    <FontAwesomeIcon icon="fa-regular fa-square" color="black" size={25} />
                                    <Text className="ml-2 text-[20px] font-semibold">{item3}</Text>
                                </View>
                                {
                                    item2.type_of_damage.map(item4 => {
                                        let filteredData = bridges.thisdamage.filter(thisitem => item4 == thisitem.code);
                                        return (
                                            <View className="mb-2">
                                                <View className="bg-sky-300 p-2 rounded-lg mb-2">
                                                    <Text className="ml-2 text-[20px] font-semibold">Code: {filteredData[0].code} - {filteredData[0].name}</Text>
                                                </View>
                                                <View>
                                                    <Text className="font-bold text-[18px] mb-1">Light</Text>
                                                    <Text className="font-bold text-[18px] mb-1">Medium</Text>
                                                    <Text className="font-bold text-[18px] mb-1">Severe</Text>
                                                    <Text className="font-bold text-[18px] mb-1">V.Severe</Text>
                                                </View>
                                            </View>
                                        )
                                    })
        ...
        return (
            <View>
                <Text className="text-black font-bold text-[20px] mb-2">{item.structure.component.name}</Text>
                {componentNames}
            </View>
        )
    }
        ...
        <View className="flex-1 justify-center bg-gray-200">
            ...
            <View className="bg-white w-[95%] self-center rounded-xl p-3">
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => <FormItems item={item} index={index} />}
